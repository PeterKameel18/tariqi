...
router.post("/", protect, async (req, res) => {
  try {
    ...
    // Block concurrent ride-requesting while the client already has another
    // in-flight ride membership/request state anywhere in the system.
    ...
    if (existingRequest) {
      return res.status(400).json({
        message:
          "You already have an active or pending ride request. Finish or cancel it before requesting another ride.",
      });
    }
    ...
    if (
      ride.passengers.some(
        (passengerId) => String(passengerId) === String(clientId),
      )
    ) {
      return res
        .status(400)
    ...
    for (const passengerId of ride.passengers) {
      const notificationToPassenger = new Notification({
        recipient: passengerId,
        type: "passenger_approval_request",
        message: "A new ride request has been sent for your ride.",
        ride: rideId,
      });
      await notificationToPassenger.save();
    }
  ...
  try {
    ...
    if (anyRejected) {
      ...
      await Ride.updateOne(
        { _id: ride._id },
        { $addToSet: { rejectedClients: joinRequest.client } },
      );
      ...
      // Add client to ride.passengers if not already present atomically
      if (
        !ride.passengers.some(
          (passengerId) => String(passengerId) === String(joinRequest.client),
        )
      ) {
        const updatedRide = await Ride.findOneAndUpdate(
          { _id: ride._id, availableSeats: { $gt: 0 } },
          {
            $addToSet: { passengers: joinRequest.client },
        ...
        if (!updatedRide) {
          ...
          return res
            .status(400)
            .json({ message: "No available seats to accept request" });
        }
      }
    ...
    const finalStatus = joinRequest.status;
    const message =
      finalStatus === "accepted"
        ? "Join request fully accepted"
        : finalStatus === "rejected"
          ? "Join request rejected"
          : "Approval recorded; awaiting other approvals";
...
// Cancel a join request by the client (soft-cancel, keep history)
router.delete("/:requestId", protect, async (req, res) => {
  try {
    ...
    if (joinRequest.status !== "pending" && joinRequest.status !== "accepted") {
      return res.status(400).json({
    ...
    if (joinRequest.status === "accepted" && ride) {
      ride.passengers = ride.passengers.filter((p) => p.toString() !== userId);
      ride.availableSeats += 1;
      await ride.save();

      const client = await require("../models/client").findById(userId);
      if (client) {
        client.inRide = null;
        client.pickup = null;
        client.dropoff = null;
    ...
    joinRequest.cancelledAt = new Date();
    joinRequest.requestedAt = null;
    await joinRequest.save();
