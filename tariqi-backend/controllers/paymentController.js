...
const confirmCashPayment = async (req, res) => {
  try {
    ...
    // Use atomic update to prevent concurrent confirmations
    ...
    if (!payment) {
      // Check why it failed to give a helpful error message
      const existingPayment = await Payment.findById(paymentId);
      if (!existingPayment) {
        return res.status(404).json({ message: "Payment not found" });
      }
      if (existingPayment.receiver.toString() !== driverId) {
      ...
      return res.status(400).json({ message: "Payment cannot be confirmed" });
    }
