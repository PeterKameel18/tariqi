...
class HomeController extends GetxController {
  ...
  ClientRidesRepo clientRidesRepo = ClientRidesRepo(dioClient: Get.find());
  ...
  Future<LatLng?> getClientLocation({required String location}) async {
    ...
    if (response != null) {
      ...
      _showSafeSnackbar("Failed", "Error Ocured While Proccessing Your Location");
    }
    return null;
  }
  ...
  Future<void> recoverActiveTripOnStartup() async {
    final didRecover = sharedPreferences.getBool('_didAutoRecoverTrip') ?? false;
    if (didRecover) return;

    final currentRoute = Get.currentRoute;
    if (currentRoute == AppRoutesNames.userTripsScreen) return;

    try {
      final response = await clientRidesRepo.getRides();
      if (!response.isRight) return;

      final rides = response.right['rides'];
      if (rides is! List) return;

      final hasRecoverableTrip = rides.any((ride) {
      ...
      if (hasRecoverableTrip) {
        await sharedPreferences.setBool('_didAutoRecoverTrip', true);
        Get.offNamed(AppRoutesNames.userTripsScreen);
      }
    } catch (_) {}
  }
