...
class DriverHomeController extends GetxController {
  ...
  bool _didAutoRecoverActiveRide = false;
  ...
  void _logRideFlow(String methodName, {String? rideId, String? extra}) {
    ...
    log("🧭 RIDE_FLOW method=$methodName rideId=${rideId ?? _driverService.currentRideId ?? 'null'} route=$route timestamp=$now extra=${extra ?? ''}");
  }
  ...
  void onReady() async {
    ...
    getDriverInfo();
    checkForActiveRide();
    _recoverActiveRideOnStartup();
  }

  Future<void> _recoverActiveRideOnStartup() async {
    if (_didAutoRecoverActiveRide) {
      _logRideFlow('DriverHomeController.recoverActiveRide.skippedAlreadyRecovered');
      return;
    }
    _didAutoRecoverActiveRide = true;
    _logRideFlow('DriverHomeController.recoverActiveRide.triggered');

    if (_driverService.consumeActiveRideRecoverySuppression(
      source: 'DriverHomeController._recoverActiveRideOnStartup',
    )) {
      _logRideFlow(
        'DriverHomeController.recoverActiveRide.suppressed',
        extra: 'User intentionally left active ride screen',
      );
      return;
    }
    ...
    if (hasActiveRide.value && activeRideId.value.isNotEmpty) {
      _logRideFlow(
        'DriverHomeController.recoverActiveRide.navigate',
        rideId: activeRideId.value,
        extra: 'Auto-recovering active ride on startup/home entry',
      );
      _driverService.startGlobalRequestPolling();
      navigateToActiveRide(activeRideId.value);
    } else {
      _logRideFlow('DriverHomeController.recoverActiveRide.noActiveRideFound');
    }
  }

  @visibleForTesting
  Future<void> recoverActiveRideOnStartupForTest() async {
    await _recoverActiveRideOnStartup();
  }
  ...
  void _setDefaultLocation() {
    ...
    // Center the map on the user's position (after FlutterMap is rendered)
    ...
    try {
      ...
      if (userPosition != null) {
        final routeResponse = await http.post(
          ...
          headers: {
            'Authorization': Secrets.openRouteServiceKey,
            'Content-Type': 'application/json',
          },
          body: jsonEncode({
            "coordinates": [
      ...
      requestState.value = RequestState.failed;
      try { Get.snackbar('Error', 'Failed to find destination: $e'); } catch (_) { }
    }
  }
  ...
  Future<void> startRide() async {
    _logRideFlow('DriverHomeController.startRide.enter');
    ...
    try {
      ...
      _logRideFlow('DriverHomeController.startRide.success', extra: 'Create ride response received');
      ...
      if (rideId != null && rideId.isNotEmpty) {
        ...
        _logRideFlow('DriverHomeController.startRide.rideIdStored', rideId: rideId);
      }
      ...
      _logRideFlow('DriverHomeController.startRide.beforeGetOff', rideId: rideId, extra: 'Navigating to DriverActiveRideScreen via Get.off');
      await Get.off(
      ...
      _logRideFlow('DriverHomeController.startRide.afterGetOff', rideId: rideId);
  ...
  void navigateToActiveRide(String? rideId) {
    try {
      _logRideFlow('DriverHomeController.navigateToActiveRide.enter', rideId: rideId);
      ...
      _logRideFlow('DriverHomeController.navigateToActiveRide.beforeGetOff', rideId: rideId, extra: 'Navigating to DriverActiveRideScreen via Get.off');
      Get.off(
        () => DriverActiveRideScreen(),
        arguments: {'rideId': rideId},
        transition: Transition.rightToLeft,
  ...
  void logout() async {
    try {
      _logRideFlow('DriverHomeController.logout.enter');
  ...
  void _safeCloseSnackbars() {
    // Intentionally avoid Get.closeAllSnackbars here.
    // In this flow it intermittently throws LateInitializationError in GetX
    // when snackbar internals aren't initialized.
  }
}
