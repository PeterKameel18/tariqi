...
class DriverService extends GetConnect {
  ...
  bool _skipNextActiveRideRecovery = false;
  
  Timer? _globalPendingRequestsTimer;
  final RxBool hasPendingRequest = false.obs;
  final RxMap<String, dynamic> pendingRequest = <String, dynamic>{}.obs;
  
  DriverService() {
    debugPrint("🔵 DRIVER_SERVICE: Constructor called - instance hashCode=$hashCode");
  }

  void suppressNextActiveRideRecovery({required String source}) {
    _skipNextActiveRideRecovery = true;
    debugPrint(
      "🧭 ACTIVE_RIDE_RECOVERY suppressNext source=$source rideId=${currentRideId ?? 'null'} route=${Get.currentRoute}",
    );
  }

  bool consumeActiveRideRecoverySuppression({required String source}) {
    final shouldSkip = _skipNextActiveRideRecovery;
    _skipNextActiveRideRecovery = false;
    debugPrint(
      "🧭 ACTIVE_RIDE_RECOVERY consume source=$source shouldSkip=$shouldSkip rideId=${currentRideId ?? 'null'} route=${Get.currentRoute}",
    );
    return shouldSkip;
  }

  void _logRideTerminationEntry(String methodName, {String? rideId}) {
    final now = DateTime.now().toIso8601String();
    final route = Get.currentRoute;
    final stack = StackTrace.current.toString();
    final stackPreview = stack.split('\n').take(8).join('\n');
    debugPrint("🧭 RIDE_TERMINATION_ENTRY method=$methodName rideId=${rideId ?? 'null'} route=$route timestamp=$now");
    debugPrint("🧭 RIDE_TERMINATION_STACK method=$methodName\n$stackPreview");
  }
    ...
    try {
      ...
      try {
        ...
        if (extractedRideId != null && extractedRideId.isNotEmpty) {
          ...
          // Start global request polling for this ride
          startGlobalRequestPolling();
        } else {
  ...
  Future<void> endAllActiveRides() async {
    _logRideTerminationEntry('DriverService.endAllActiveRides', rideId: currentRideId);
    final token = Get.find<AuthController>().token.value;
    if (token.isEmpty) {
  ...
  Future<bool> endRide(String rideId) async {
    try {
      _logRideTerminationEntry('DriverService.endRide', rideId: rideId);
      log("📤 Attempting to end ride with ID: $rideId");
      
      final authController = Get.find<AuthController>();
      final token = authController.token.value;
      
      // Backend route is POST /driver/end/ride/:rideId
      final url = "${ApiEndpoints.driverEndRide}/$rideId";
      ...
      if (response.statusCode == 200) {
        // Stop global request polling
        stopGlobalRequestPolling();
  ...
  void startGlobalRequestPolling() {
    if (_globalPendingRequestsTimer != null && _globalPendingRequestsTimer!.isActive) {
      log("⚠️ Global request polling already active");
      return;
    }
    
    if (currentRideId == null || currentRideId!.isEmpty) {
      log("⚠️ Cannot start global polling: no active ride ID");
      return;
    }
    
    log("🔔 Starting global pending request polling for ride: $currentRideId");
    
    _globalPendingRequestsTimer = Timer.periodic(
      const Duration(seconds: 20),
      (_) => _fetchAndNotifyPendingRequests(),
    );
    
    _fetchAndNotifyPendingRequests();
  }
  
  void stopGlobalRequestPolling() {
    log("🔕 Stopping global pending request polling");
    _globalPendingRequestsTimer?.cancel();
    _globalPendingRequestsTimer = null;
    hasPendingRequest.value = false;
    pendingRequest.clear();
  }
  
  Future<void> _fetchAndNotifyPendingRequests() async {
    if (currentRideId == null || currentRideId!.isEmpty) {
      log("⚠️ No active ride, skipping pending request check");
      return;
    }
    
    if (hasPendingRequest.value) {
      log("ℹ️ Already have pending request, skipping fetch");
      return;
    }
    
    try {
      final requests = await getPendingRequests(currentRideId!);
      
      if (requests.isEmpty) {
        return;
      }
      
      log("✅ Global polling found ${requests.length} pending requests");
      
      final request = requests.first;
      final requestId = (request['_id'] ?? request['id'] ?? '').toString();
      ...
      final pickupLocationName = await LocationDisplayHelper.resolveLabel(
        request['pickup'],
        fallback: 'Current location',
      );
      final dropoffLocationName = await LocationDisplayHelper.resolveLabel(
        request['dropoff'],
      ...
      debugPrint("🔴 GLOBAL_POLLING: Detected pending request id=$requestId");
      debugPrint("🔴 GLOBAL_SERVICE: Before toggle - hasPendingRequest=${hasPendingRequest.value}");
      
      // Force toggle to ensure reactive listener fires
      hasPendingRequest.value = false;
      debugPrint("🔴 GLOBAL_SERVICE: Set to FALSE (instance hashCode=$hashCode)");
      
      await Future.delayed(const Duration(milliseconds: 50));
      
      hasPendingRequest.value = true;
      debugPrint("🔴 GLOBAL_SERVICE: Set to TRUE (instance hashCode=$hashCode)");
      debugPrint("🔴 GLOBAL_SERVICE: After toggle - hasPendingRequest=${hasPendingRequest.value}");
      debugPrint("🔴 GLOBAL_SERVICE: pendingRequest id=${pendingRequest['id']}");
      log("🔔 Global pending request set: $requestId");
      
      Future.delayed(const Duration(seconds: 30), () {
        if (hasPendingRequest.value && pendingRequest['id'] == requestId) {
          log("⏰ Auto-clearing global pending request after timeout");
          hasPendingRequest.value = false;
          pendingRequest.clear();
        }
      });
    } catch (e) {
      log("❌ Error in global pending request fetch: $e");
    }
  }
  
  void clearPendingRequest() {
    debugPrint("🔴 GLOBAL_SERVICE: clearPendingRequest called");
    hasPendingRequest.value = false;
    pendingRequest.clear();
    log("🧹 Global pending request cleared");
  }
  ...
  Future<Map<String, dynamic>> approveJoinRequest(
    ...
    try {
      ...
      log("📤 Request URL: $url");
      debugPrint("DRIVER_SVC request requestId=$requestId method=PUT url=$url");
      ...
      log("📥 Response body: ${response.body}");
      debugPrint(
        "DRIVER_SVC response requestId=$requestId method=PUT url=$url statusCode=${response.statusCode} body=${response.body}",
      );
      ...
      if (response.statusCode == 200) {
        try {
          ...
          if (data is Map<String, dynamic>) {
            final dynamic rawFinalStatus = data["finalStatus"] ?? data["status"];
            final String? finalStatus = rawFinalStatus is String ? rawFinalStatus : null;
            final bool actionApplied =
                data["actionApplied"] == true || finalStatus != null;
            debugPrint(
              "DRIVER_SVC parsed requestId=$requestId actionApplied=$actionApplied finalStatus=$finalStatus message=${data["message"]}",
            );
          ...
          log("⚠️ Error parsing approveJoinRequest success response: $e");
        }
      }
...
class ChatService {
  ...
  static Future<void> createChatRoom(String token, String rideId) async {
    ...
    log('🟩 Chat create room response (${response.statusCode}): ${response.body}');
